...
export function* createApplicationSaga(action: ReduxAction<CreateApplicationPayload & { containingFolderId: string }>) {
  try {
    ...
    if (isValidResponse) {
      yield put({
        ...
        payload: { ...response.data.data, containingFolderId: action.payload.containingFolderId },
      });
