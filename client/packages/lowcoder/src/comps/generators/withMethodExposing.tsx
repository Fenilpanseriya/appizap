...
export function withMethodExposingBase<
  T extends ExposeMethodCompConstructor<AbstractComp>,
>(
  VariantComp: T,
  methodConfigs:
    | MethodConfigInfo<T>[]
  ...
  return class extends VariantComp {
    ...
    executeMethod(action: CustomAction<ExecuteAction>) {
      ...
      const method = this.getMethodConfigInfo().find(
...
export function withMethodExposing<
  T extends ExposeMethodCompConstructor<MultiBaseComp>,
>(VariantComp: T, methodConfigs: MethodConfigsType<T>) {
      ...
      if (isMyCustomAction<ExecuteAction>(action, "execute")) {
        if (!this.executeMethod(action)) {
          ...
          Object.values(this.children).forEach(
            (child) =>
              isExposingMethodComp(child) && child.executeMethod(action)
          );
        }
        return this;
      }
...
export function isExposingMethodComp(
  comp: Comp | ExposingMethodComp
): comp is ExposingMethodComp {
...
export function refMethods<T>(
  ...
  ExposeMethodCompConstructor<MultiBaseComp<{ viewRef: RefControl<T> }>>
>[] {
  return methods.map((method) => ({
    method,
    execute: (
      comp: MultiBaseComp<{ viewRef: RefControl<T> }>,
