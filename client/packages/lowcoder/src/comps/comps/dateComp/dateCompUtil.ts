...
export const disabledDate = (current: dayjs.Dayjs, min: string, max: string, disabledDates?: Array<String>, format: string = 'YYYY-MM-DD', disabledFutureDates: boolean = false, disabledPastDates: boolean = false) => {
  ...
  const currentDateString = current.format(format ? format : 'YYYY-MM-DD');

  if (
    (disabledFutureDates && current.isAfter(dayjs(), "date")) ||
    (disabledPastDates && current.isBefore(dayjs(), "date"))
  ) {
    return true; // Disable future or past dates based on flags
  }
  ...
  if (disabledDates && disabledDates.some((date: any) => date === currentDateString)) {
    return true; // Disable dates in the array
  }
