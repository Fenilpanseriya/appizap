...
function isStrokeWidthConfig(
  config: SingleColorConfig
): config is StrokeWidthConfig {
  return config.hasOwnProperty("strokeWidth");
}

function isCounterClockwiseConfig(
  config: SingleColorConfig
): config is CounterClockwiseConfig {
  return config.hasOwnProperty("counterClockwise");
}

function isSizeConfig(config: SingleColorConfig): config is SizeConfig {
  return config.hasOwnProperty("size");
}

function isLineHeightConfig(
  config: SingleColorConfig
): config is LineHeightConfig {
  return config.hasOwnProperty("lineHeight");
}

function isTextIndentConfig(
  config: SingleColorConfig
): config is TextIndentConfig {
  return config.hasOwnProperty("textIndent");
}

function isLetterSpacingConfig(
  config: SingleColorConfig
): config is LetterSpacingConfig {
  return config.hasOwnProperty("letterSpacing");
}

function isWordSpacingConfig(
  config: SingleColorConfig
): config is WordSpacingConfig {
  return config.hasOwnProperty("wordSpacing");
}

function isFontVariantConfig(
  config: SingleColorConfig
): config is FontVariantConfig {
  return config.hasOwnProperty("fontVariant");
}
...
function isEmptyStrokeWidth(strokeWidth: string) {
  return _.isEmpty(strokeWidth);
}
...
function isEmptyCounterClockwise(counterClockwise: string) {
  return _.isEmpty(counterClockwise);
}

function isEmptySize(size: string) {
  return _.isEmpty(size);
}

function isEmptyLineHeight(lineHeight: string) {
  return _.isEmpty(lineHeight);
}
function isEmptyTextIndent(textIndent: string) {
  return _.isEmpty(textIndent);
}
function isEmptyLetterSpacing(letterSpacing: string) {
  return _.isEmpty(letterSpacing);
}
function isEmptyWordSpacing(wordSpacing: string) {
  return _.isEmpty(wordSpacing);
}
function isEmptyFontVariant(fontVariant: string) {
  return _.isEmpty(fontVariant);
}
/**
...
function calcColors<ColorMap extends Record<string, string>>(
  ...
  colorConfigs.forEach((config) => {
    ...
    if (!isEmptyStrokeWidth(props[name]) && isStrokeWidthConfig(config)) {
      res[name] = props[name];
      return;
    }
    ...
    if (
      !isEmptyCounterClockwise(props[name]) &&
      isCounterClockwiseConfig(config)
    ) {
      res[name] = props[name];
      return;
    }
    if (!isEmptySize(props[name]) && isSizeConfig(config)) {
      res[name] = props[name];
      return;
    }
    ...
    if (!isEmptyTextIndent(props[name]) && isTextIndentConfig(config)) {
      res[name] = props[name];
      return;
    }
    if (!isEmptyLetterSpacing(props[name]) && isLetterSpacingConfig(config)) {
      res[name] = props[name];
      return;
    }
    if (!isEmptyWordSpacing(props[name]) && isWordSpacingConfig(config)) {
      res[name] = props[name];
      return;
    }
    if (!isEmptyFontVariant(props[name]) && isFontVariantConfig(config)) {
      res[name] = props[name];
      return;
    }
    ...
    if (isStrokeWidthConfig(config)) {
      res[name] = themeWithDefault[config.strokeWidth] || "8px";
    }
    if (isCounterClockwiseConfig(config)) {
      res[name] = themeWithDefault[config.counterClockwise] || "false";
    }
    if (isLineHeightConfig(config)) {
      res[name] = themeWithDefault[config.lineHeight] || "1.5";
    }
    if (isTextIndentConfig(config)) {
      res[name] = themeWithDefault[config.textIndent] || "0px";
    }
    if (isLetterSpacingConfig(config)) {
      res[name] = themeWithDefault[config.letterSpacing] || "0px";
    }
    if (isWordSpacingConfig(config)) {
      res[name] = themeWithDefault[config.wordSpacing] || "0px";
    }
    if (isFontVariantConfig(config)) {
      res[name] = themeWithDefault[config.fontVariant] || "normal";
    }
  });
...
const RadiusIcon = styled(IconRadius)`
  margin: 0 8px 0 -2px;
`;
const BorderIcon = styled(BorderWidthIcon)`
  margin: 0 8px 0 -3px;
  padding: 3px;
`;
...
const StyledTextWeightIcon = styled(TextWeightIcon)`
  ...
  padding: 3px;
`;
const StyledBackgroundImageIcon = styled(ImageCompIcon)`
  margin: 0 0px 0 -12px;
`;
                                    ...
                                    : name === "fontFamily"
                                      ...
                                      : name === "fontStyle"
                                        ? (
                                            ...
                                            label: config.label,
                                            preInputNode: (
                                              <StyledFontFamilyIcon title="Font Style" />
                                            ),
                                            placeholder: props[name],
                                          })
                                        : name === "backgroundImage" ||
                                          ...
                                          : name === "backgroundImageRepeat" ||
                                            ...
                                            : name === "backgroundImageSize" ||
                                              ...
                                              ? (
                                                  ...
                                                  label: config.label,
                                                  preInputNode: (
                                                    <StyledBackgroundImageIcon title="Background Image Size" />
                                                  ),
                                                  placeholder: props[name],
                                                })
                                              : name ===
                                                ...
                                                ? (
                                                    ...
                                                    label: config.label,
                                                    preInputNode: (
                                                      <StyledBackgroundImageIcon title="Background Image Position" />
                                                    ),
                                                    placeholder: props[name],
                                                  })
                                                : name ===
                                                  ...
                                                  ? (
                                                      ...
                                                      label: config.label,
                                                      preInputNode: (
                                                        <StyledBackgroundImageIcon title="Background Image Origin" />
                                                      ),
                                                      placeholder: props[name],
                                                    })
                                                  : name === "boxShadow" ||
                                                    ...
                                                    : name === "size"
                                                      ...
                                                      : name === "lineHeight" ||
                                                        ...
                                                        ? (
                                                            ...
                                                            label: config.label,
                                                            preInputNode: (
                                                              <StyledTextSizeIcon title="Text Indent" />
                                                            ),
                                                            placeholder:
                                                              props[name],
                                                          })
