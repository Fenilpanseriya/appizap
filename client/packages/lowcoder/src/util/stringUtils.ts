...
export const getNextEntityName = (
  ...
  if(prefix.startsWith("New Workspace (Organization)")){
    let usedIndexCount=0

    for (let i = 0; i < existingNames.length; i++) {
      if (existingNames[i].startsWith("New Workspace (Organization)")) {
        usedIndexCount++;
      }
    }
    return prefix + (usedIndexCount + 1);
  }
  
  const regex = new RegExp(`^${prefix}(\\d+)$`);
  console.log(prefix,existingNames,startWithoutIndex)
  ...
  const usedIndices: number[] = findIndexs(existingNames)

  function findIndexs(array:string[]){
    const secondIndex = array.findIndex((item, index) => index > 0 && (item === "Home Page"));
    console.log("second index is "+secondIndex)
    if (secondIndex !== 0) {
  // Slice the array up to the index of the second occurrence of "Home Page"
      let resultArray=[]
