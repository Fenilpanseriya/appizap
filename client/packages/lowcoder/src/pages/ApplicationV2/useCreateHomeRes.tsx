...
import { useCreateCustomApp } from "./useCreateCustomApp";
...
export function useCreateHomeRes() {
  ...
  const handleCustomAppCreate = useCreateCustomApp();
  ...
  const handleCreate = useCallback(
    (type: HomeResTypeEnum) => {
      ...
      if (type === HomeResTypeEnum.Folder) {
        handleFolderCreate();
      } else if (type === HomeResTypeEnum.Application) {
        handleCustomAppCreate();
      } else {
        ...
        const nextNewApplicationName = getNextEntityName(
          newAppPrefix(user.username, applicationType),names
        );
        const dsl = {
          ui: {
