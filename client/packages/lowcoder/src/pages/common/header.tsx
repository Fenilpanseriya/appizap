...
import { currentApplication, getParentFolderId } from "redux/selectors/applicationSelector";
import {
...
const IconCss = css<{ $show?: boolean }>`
  ...
  & g g {
    stroke: ${(props) => (props.$show ? "#000000" : "#dddddd")};
  }
...
const ModeButton = styled(Radio.Button)`
background: ${(props) => (props.checked ? '#00AA92' : 'white')};
`;
    ...
    <>
      ...
      >
        {editorModeOptions.map((option) => (
          <ModeButton key={option.key} value={option.value}>
            {option.label}
          </ModeButton>
        ))}
      </Radio.Group>
