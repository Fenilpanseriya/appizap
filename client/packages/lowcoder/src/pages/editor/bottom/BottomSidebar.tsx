...
import { TogglePanel, PanelStatus } from "@lowcoder-ee/pages/common/header";
...
const CollapseIcon = styled(Packup)<Irotate>`
  height: 24px;
  width: 24px;
  margin-left: 6px;
...
export function BottomSidebar(props: BottomSidebarProps) {
  const { items, refTreeComp, onOpenCreatePanel, onSelect, onCopy, onDelete, togglePanel, panelStatus } = props;
  ...
  return (
    ...
    <Contain style={props.style}>
      <Title>
        ...
        <TitleRight>
          ...
          <CollapseIcon  onClick={() => togglePanel("bottom")} deg="rotate(180deg)"/>
        </TitleRight>
      </Title>
      {isSearchShow && (
        <SearchWrapper>
    ...
    <Title>
      ...
      <TitleRight>
        <CollapseIcon  onClick={() => togglePanel("bottom")} deg="rotate(0deg)"/>
      </TitleRight>
    </Title>
  </Contain> 
  );
...
export const EmptyQueryList = (props: { newColumn: () => void }) => (
  <div style={{ display: "flex" }}>
    <NolistDiv>
      ...
      <span
        style={{ color: "#00aa92", cursor: "pointer", margin: "0 4px" }}
