...
import { getParentFolderId, isApplicationCreating, normalAppListSelector } from "@lowcoder-ee/redux/selectors/applicationSelector";
import { createApplication } from "@lowcoder-ee/redux/reduxActions/applicationActions";
import { AppPathParams, AppTypeEnum, AppUILayoutType, ApplicationDetail, ApplicationMeta } from "@lowcoder-ee/constants/applicationConstants";
import { getNextEntityName } from "@lowcoder-ee/index.sdk";
import { newAppPrefix } from "../ApplicationV2/useCreateHomeRes";
import { getUser } from "@lowcoder-ee/redux/selectors/usersSelectors";
import { folderElementsSelector, foldersSelector } from "@lowcoder-ee/redux/selectors/folderSelector";
...
const Page = styled.div<{ isactive: string }>`
  width: 100%;
  height: 100%;
  background-color: ${(p) => (p.isactive == "true" ? "#f2f7fc" : "transparent")};
  padding: 0 14px;
  font-size: 14px;
  display: flex;
  align-items: center;
...
export const LeftContent = (props: LeftContentProps) => {
  ...
  // this returns the current folder id
  const parentApplicationId = useSelector(getParentFolderId);

  // stores the current folder
  const currentApplication = useSelector((state: AppState) => {
   
    const currentFolder = state.ui.folder.folders.find((f) => {console.log(f);return f.folder && f.folderId === parentApplicationId});
    return currentFolder;
  }, shallowEqual);

  const applicationListNames = useSelector((state: AppState) => {
    const names: string[] = [];
    state.ui.folder.folders.forEach((f) => {
      if (f.folder) {
        f.subApplications?.forEach((a) => {
          names.push(a.name);
        })
      }
    });
  ...
  // this returns the applications present in the container folder
  const pagesInApplication = currentApplication?.subApplications?.sort((a, b) => a.createAt - b.createAt) || [];
  ...
  const handlePageClick = (page: ApplicationMeta) => {
    history.push(buildAppRouteWithState(page.applicationId, false));
  }
  ...
  const showPages = () => {
    ...
    return <div>
      {pagesInApplication?.map((page) => {
        return <Page key={page.applicationId} onClick={() => handlePageClick(page)} isactive={page.applicationId === applicationId ? "true" : "false"}>
          {/* {page.name==="Home Page"?page.name:page.name.split("'s")[1]}
           */}
           {
            page.name.includes(user.username)?page.name.split("'s")[1]:page.name
           }
        </Page>
      })}
    </div>
  ...
  // create a page inside application
  const createApp = () => {
    if (isCreating) {
      return;
    }
    const applicationType = AppTypeEnum.Application;
    ...
    const nextNewApplicationName = getNextEntityName(
      ...
      applicationListNames,user.username
    );
    ...
    if (parentApplicationId) {
      dispatch(
        createApplication({
          ...
          dsl,
          folderId: parentApplicationId,
          containingFolderId: parentApplicationId,
          onSuccess: (app: ApplicationDetail) => {
          },
        })
      );
    } else {
      console.error('can\'t create application in root folder');
    }
  }
  ...
  const stateContent = (
    <ScrollBar>
      <div style={{ paddingBottom: 80 }}>
        <BaseSection name={'Pages'} width={288} noMargin createPage={createApp}>
          <span className={leftCompListClassName}>{showPages()}</span>
        </BaseSection>
